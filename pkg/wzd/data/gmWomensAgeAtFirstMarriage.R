gmWomensAgeAtFirstMarriage <- local({
  if (!require(xts)) stop('Could not load required library xts')
  dataCsv <- 'gmWomensAgeAtFirstMarriage.csv'
  if (!file.exists(dataCsv))
    if (download.file('http://spreadsheets.google.com/pub?key=t4eF8H_jq_xyKCUHAX6VT1g&gid=0&output=csv', dataCsv, method='internal')!=0) stop('Could not download dataset.')
