gmPopulation <- local({
  ...
  dataCsv <- 'gmPopulation.csv'
  if (!file.exists(dataCsv))
    if (download.file('http://spreadsheets.google.com/pub?key=phAwcNAVuyj0XOoBL_n5tAQ&gid=0&output=csv', dataCsv, method='internal')!=0) stop('Could not download dataset.')
