gmCellPhones <- local({
  ...
  dataCsv <- 'gmCellPhones.csv'
  if (!file.exists(dataCsv))
    if (download.file('http://spreadsheets.google.com/pub?key=pyj6tScZqmEcKuNdFCUo6TQ&gid=0&output=csv', dataCsv, method='internal')!=0) stop('Could not download dataset.')
