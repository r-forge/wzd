gmIncomePerCapita <- local({
  ...
  dataCsv <- 'gmIncomePerCapita.csv'
  if (!file.exists(dataCsv))
    if (download.file('http://spreadsheets.google.com/pub?key=phAwcNAVuyj1jiMAkmq1iMg&gid=0&output=csv', dataCsv, method='internal')!=0) stop('Could not download dataset.')
