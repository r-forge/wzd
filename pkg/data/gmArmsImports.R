gmArmsImports <- local({
  ...
  dataCsv <- 'gmArmsImports.csv'
  if (!file.exists(dataCsv))
    if (download.file('http://spreadsheets.google.com/pub?key=pyj6tScZqmEfnPl7VRfT9WA&gid=0&output=csv', dataCsv, method='internal')!=0) stop('Could not download dataset.')
