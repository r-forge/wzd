gmSurfaceArea <- local({
  ...
  dataCsv <- 'gmSurfaceArea.csv'
  if (!file.exists(dataCsv))
    if (download.file('http://spreadsheets.google.com/pub?key=pyj6tScZqmEeiMy8j86qDTg&gid=0&output=csv', dataCsv, method='internal')!=0) stop('Could not download dataset.')
