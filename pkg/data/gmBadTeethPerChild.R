gmBadTeethPerChild <- local({
  if (!require(xts)) stop('Could not load required library xts')
  dataCsv <- 'gmBadTeethPerChild.csv'
  if (!file.exists(dataCsv))
    if (download.file('http://spreadsheets.google.com/pub?key=phAwcNAVuyj3Os9LVO_pRDA&gid=0&output=csv', dataCsv, method='internal')!=0) stop('Could not download dataset.')
